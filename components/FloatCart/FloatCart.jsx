...
import { getCartTotal, getCartState, getIsOpen } from '../../store/selectors/products'
      ...
      <div className={classes.join(' ')}>
        ...
        <div className="float-cart__content">
          ...
          <div className="float-cart__shelf-container">
            {cart.map((pv) => {
              return <CartProduct product_variation={pv} key={pv.external_id} />
            })}

            {cart.length === 0 && <p className="shelf-empty">Adicione algum produto no carrinho</p>}
          </div>
          <div className="float-cart__footer">
            <div className="sub">TOTAL</div>
            <div className="sub-price">
              <p className="sub-price__val">{`R$${total.toFixed(2).replace('.', ',')}`}</p>

              {cart.length > 0 && (
                <span className="sub-price__installment">
                  {`6x de R$${(total / 6)
                    .toFixed(2)
                    .replace('.', ',')} sem juros no cartão ou 12% de desconto no boleto e pix`}
                </span>
              )}
            </div>
            <div
