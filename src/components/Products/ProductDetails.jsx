...
const ProductDetails = ({ product }) => {
    ...
    useEffect(() => {
        let tamanhos = variacoes
            .map(el => el.nome.split(";").slice(1, 2)[0].split(":").slice(1, 2)[0])
    ...
    const onSelectedSizeChange = (value) => {
        ...
        const variacaoDisponivel = variacoes
            .filter(el => el.nome.split(";").slice(1, 2)[0].split(":").slice(1, 2)[0] == value)
        ...
        if (codigoProduto) {
            if (selectedSize === "" || selectedColor === "") {
                MySwal.fire({
                    title: (
                        <p>
                            Selecione um tamanho e uma cor para adicionar ao
                            carrinho
                        </p>
                    ),
                ...
                <div className="details-content">
                    ...
                    {codigoProduto ? (
                        <div className="sizes-btn">
                            <ul>
                                {sizes.map((value) => (
                                    <li onClick={() => onSelectedSizeChange(value)} 
                                        key={value}
                    ...
                    {availableVariations ? (
                        <div className="colors-product">
                            <span>
                                <b>Cor:</b>
                                <div className="color-text-selected">{selectedColor.charAt(0).toUpperCase() + selectedColor.toLowerCase().slice(1)}</div>
                            </span>
